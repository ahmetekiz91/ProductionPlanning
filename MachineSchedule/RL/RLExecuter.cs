...
{
    public class RLExecuter
    {
        public RLExecuter() { }

        public void RLExecute()
        {
            DateTime startTime = DateTime.Now;
            AzadContext context = new AzadContext();

            RLSolver model = new RLSolver(context, startTime);
            model.run(startTime, 2000).GetAwaiter().GetResult();
            ...
            FROM [MachineSchedule] where [Algorithm]='RL' ").Single();
